    ...
    public IEnumerator ECharge()
    {
        ...
        while (true)
        {
            ...
            if (Input.GetKey(KeyCode.Space))
            {
                ...
                {
                    var bullet = K.Shot<NormalBullet>(ePOOL_TYPE.NormalBullet, transform.position + shapes[i], shapes[(i + 2) % shapes.Count].normalized, 0, 50, 200000);
            ...
            else if (Input.GetKeyUp(KeyCode.Space))
            {
                for (int i = 0; i < ((int)edgeCount + 2); i++)
                {
                    ...
                    for (int j = 0; j < GameManager.Instance.Level; j++)
                    {
                        ...
                        if (!near) continue;
                        var bezier = K.Shot<BezierBullet>(ePOOL_TYPE.BezierBullet, K.player.transform.position, Vector3.zero, K.player.damage + K.player.chargeDamage, 200);
